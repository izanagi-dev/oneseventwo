...
func _on_area_entered(area: Area2D) -> void:
	...
	# Ignore self hits (Hitbox is a child of its owner; Hurtbox is also a child of owner)
	...
	# Preferred: Hurtbox script
	if area.has_method("apply_hit"):
		area.apply_hit(dmg, get_parent())
		return

	# Fallback: call receive_hit() on the Area's owner if it exists
